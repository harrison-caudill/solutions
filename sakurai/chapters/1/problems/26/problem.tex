...
\begin{answer}
  ...
  \begin{answerpart}{The World is Spinoring}
    ...
    Recalling equation \outref{1}{11}{spinAngle} and looking at figure
    \outref{1}{11}{spinAxes} we can rotate around the $x$ axis by
    setting $\beta$ to $-\pi/2$ and setting $\alpha$ to $-\pi/2$ so
    that our rotation is about the $x$ axis and a negative value for
    $\beta$ results in a clockwise rotation.
    ...
    back at our solution from problem \outref{1}{11}{theQuestion}.  We
    ...
    derivation from problem \outref{1}{11}{theQuestion} that depends
    upon this assumption of range, but the sign issue is there
    nonetheless.  Let's see what happens when we correct for that by
    going $\pi/2$ in the other direction instead.
    ...
    equation 3.62 (which we actually derived from first principles
    back in equation \outref{1}{5}{taylor}), plug in $\phi$ and call
    it a day
  ...
  \begin{answerpart}{\texorpdfstring{Building $Z$'s from $Y$'s}{Building Z's from Y's}}
    ...
    Referencing equation \outref{1}{7}{theMatrixHasYouNow} to build
    our matrix, we have:
