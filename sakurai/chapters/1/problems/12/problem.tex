...
\begin{purpose}
  This is practice with eigenvectors.  Interestingly, though, his
  choice of matrix on which to practice isn't entirely random.  It's a
  Hadamard gate from quantum computing which is a sum of $\sigma_x$
  and $\sigma_z$.  Sakurai doesn't really develop the meaning of the
  Pauli spin operators, instead teaching us to use them to do other
  things to themselves and occasionally notice that one snuck in
  there.  So what exactly it means to add two Pauli spin operators
  together is left to our own imaginations.  For what it's worth, in
  Quantum computing, the Hadamard gate takes in your nicely aligned
  QBit and puts it into an equal superposition of $\ket{1}$ and
  $\ket{2}$.

  On an historical note, the original book predates the rise of
  quantum computing, but not necessarily the development of the
  Hadamard gate.  The ``Revised Edition'' that I used back in
  undergrad was published in 1994 and has the same problem (though
  it's labeled as 11) and the photocopy of the solutions manual that I
  found for that edition does not mention the Hadamard gate.  This
  realization is all post-factum and originally this problem was just
  viewed as cranking through an uninteresting eigenvector computation.
  Maybe you'll have more fun with it than I did, and submit a pull
  request.
\end{purpose}
...
\begin{answer}
  ...
  \begin{equation}
    \begin{split}
      N_1 &= \frac{1}{\norm{\vec{a_1}}}\\
      &= \frac{1}{\sqrt{\vec{a_1} \cdot \vec{a_1}}}\\
      &= \frac{1}{\sqrt{4 - 2\sqrt{2}}}\\
      N_2 &= \frac{1}{\norm{\vec{a_2}}}\\
      &= \frac{1}{\sqrt{\vec{a_2} \cdot \vec{a_2}}}\\
      &= \frac{1}{\sqrt{4 - 2\sqrt{2}}}\\
      N &= N_1 = N_2 = \frac{1}{\sqrt{4 - 2\sqrt{2}}}\\
    \end{split}
  \end{equation}
