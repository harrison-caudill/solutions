\begin{question}
  ...
  \begin{questionpart}
    ...
    \end{equation*}
    is an eigenstate of the coordinate operator x.  What is the
...
\begin{answer}
  ...
  \begin{answerpart}{All Together Now}
    Showing the eigen-nature of $\mathscr{J}(-a)$ in the $x$ basis is
    now much simpler. \textbf{WARNING:} The problem gives us an
    exponential form of the jog operator with a \emph{negative} jog.
    That's why you see a $-a$ there instead of $a$.  This is a giant
    pain that was discovered at the $22^{\text{nd}}$ hour and I'm paging this
    back in dealing with tracing a sign error because Sakurai put a
    negative in there.
    ...
    \begin{equation}
      \begin{alignedat}{2}
        ...
        \implies
        &&x\mathscr{J}(-a)
        &= \mathscr{J}(-a)x - a\mathscr{J}(-a)
        \\
        \implies
        &&x\mathscr{J}(-a)\ket{x'}
        &= (\mathscr{J}(-a)x - a\mathscr{J}(-a))\ket{x'}
        \\
        &&&= (x' - a)\mathscr{J}(-a)\ket{x'}
        \\
      \end{alignedat}
    \end{equation}

    implying an eigenvalue of $x' - a$.
  \end{answerpart}
