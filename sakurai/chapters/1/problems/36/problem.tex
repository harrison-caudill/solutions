\begin{question}
  ...
  \begin{questionpart}
    ...
    \begin{questionsubpart}
      \begin{equation*}
        \mel{\beta}{x}{\alpha} = \int \dd{p'} \phi_{\beta}^{*}(p')i \hbar \pdv{}{p'} \phi_{\alpha}(p')
      \end{equation*}
    \end{questionsubpart}
...
\begin{purpose}
  This one is actually pretty cool.  The process of hypothesizing the
  existence of something, and then using the desired properties to end
  up deriving the form it must take is instructive both in terms of
  why it has the form it does but also because that process is itself
  a \emph{very} useful tool.
\end{purpose}
...
\begin{answer}
  \begin{answerpart}{Oh yeah?  Prove it.}
    ...
    \begin{answerspart}
      ...
      \begin{equation}
        \begin{split}
          \mel{\beta}{x}{\alpha}
          &= \int \dd{p'} (\bra{\beta})(\dyad{p'})(x\ket{\alpha})\\
      ...
      \begin{equation}
        \begin{split}
          \mel{\beta}{x}{\alpha}
          &= \int \dd{p'} \braket{\beta}{p'}\mel{p'}{x}{\alpha}\\
          &= \int \dd{p'} \braket{\beta}{p'}\left(i\hbar \pdv{}{p'}\right)\braket{p'}{\alpha}\\
          &= \int \dd{p'} \braket{\beta}{p'}\left(i\hbar \pdv{}{p'}\right)\braket{p'}{\alpha}\\
        \end{split}
      \end{equation}
  ...
  \begin{answerpart}{Let's Get Physical}
    ...
    lamest answer for the last one.  Another way of saying that is
    that it is

    \begin{equation}
      \exp(\frac{ix\Xi}{\hbar})
      = \text{SELECT * FROM Hilbert WHERE p=$\Xi$};
    \end{equation}
  ...
  this problem \emph{before} solving problem
  \outref{1}{24}{theQuestion}, so look somewhere else for a
  celebration.
