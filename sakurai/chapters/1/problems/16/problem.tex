...
\begin{purpose}
  As we move away from grind-it-out problems using the basic tools of
  quantum mechanics and towards a deeper exploration of how the
  electron \emph{feels} and learn to not just listen but
  to... \emph{listen}, we start getting things that are a bit more
  open-ended.  In this case, we're given an operator and a couple of
  clues about how to poke and prod at it to learn its secrets.  Given
  that a physicist's job is poke and prod at the universe to learn
  \emph{its} secrets, you could reasonably view this as apprenticing.
\end{purpose}
...
\begin{answer}
  ...
  \begin{answerpart}{Interesting Eigenvalues \& Rotations}
    ...
    back our $c_0$ as part of it.  We're going to distinguish, now,
    between the scaled and unscaled eigenvalues.  So while $\lambda =
    \pm \sqrt{2}$ for the eigenvalues whose existences are unsullied
    by $c_0$ we'll assume that $\lambda' = \pm c_0\sqrt{2} = \pm 1$.
    
    You're a big (statistically-speaking) boy now and you can go and
    get a scaling factor of $c_0$ on your own if you need one.
    Solving for the unscaled eigenvectors using $\lambda$
